...
- **แนวคิดหลัก:** Kubernetes คือระบบกำกับดูแลที่ทำให้การทำงานของแอปสอดคล้องกับสภาพที่กำหนดไว้อย่างต่อเนื่อง แม้เครื่องหรือแอปจะหยุดทำงาน
...
- **แนวคิดหลัก:** ทรัพยากรทั้งหมดใน Kubernetes อยู่ในรูปแบบ object ซึ่งกำหนดผ่าน kubectl และจัดกลุ่มด้วย namespace
...
- **แนวคิดหลัก:** หน่วยที่เล็กที่สุดซึ่ง Kubernetes กำกับดูแลคือ Pod ไม่ใช่ container
...
- **แนวคิดหลัก:** ผู้ใช้กำหนดสภาพที่ต้องการแทนการระบุขั้นตอนดำเนินงาน แล้ว Kubernetes จัดการให้สอดคล้องกับข้อกำหนดดังกล่าว (declarative)
...
## 005-labels-the-glue
- **แนวคิดหลัก:** label เป็นกลไกเชื่อมโยง object โดยไม่ต้องอ้างอิงชื่อหรือ IP ของแต่ละ object โดยตรง
...
- **แนวคิดหลัก:** Kubernetes เปรียบเทียบจำนวนที่กำหนดไว้กับจำนวนที่มีอยู่จริงอย่างต่อเนื่อง และปรับแก้ส่วนต่างโดยอัตโนมัติ
...
- **แนวคิดหลัก:** Deployment เป็นชั้นที่กำกับการเปลี่ยนแปลง โดยรองรับการ scale และเปลี่ยนเวอร์ชันโดยไม่ต้องลบทรัพยากรเดิมด้วยตนเอง
...
- **แนวคิดหลัก:** เมื่อ Pod ถูกสร้างใหม่ IP จะเปลี่ยนแปลง จึงต้องมี "ชื่อคงที่" สำหรับใช้อ้างอิงแทน
...
- **แนวคิดหลัก:** Service มีหลายชนิดซึ่งแตกต่างกันด้านขอบเขตของผู้ที่สามารถเข้าถึงได้ ไม่ใช่กลไกการทำงาน
...
- **แนวคิดหลัก:** ไม่ควรบันทึก config ไว้ใน image เพื่อให้ build เพียงครั้งเดียวและเปลี่ยนพฤติกรรมได้ตามสภาพแวดล้อมที่ deploy
...
- **แนวคิดหลัก:** ต้องแยกข้อมูลลับออกจากข้อมูลทั่วไป และ base64 ไม่ใช่การเข้ารหัส
...
- **แนวคิดหลัก:** ข้อมูลที่เขียนไว้ใน container จะสูญหายเมื่อ Pod ถูกสร้างใหม่ เนื่องจาก container ถูกสร้างใหม่จาก image เสมอ
...
- **แนวคิดหลัก:** หากต้องการให้ข้อมูลคงอยู่ ต้องบันทึกข้อมูลไว้ภายนอก container โดยขอพื้นที่ผ่าน PVC
...
- **แนวคิดหลัก:** สถานะ "Pod Running" ไม่ได้หมายความว่าแอปพลิเคชันพร้อมรับงาน
...
- **แนวคิดหลัก:** แอปพลิเคชันจริงประกอบด้วยหลาย object ที่ทำงานร่วมกัน มิใช่ object เดี่ยว
...
- **แนวคิดหลัก:** องค์ประกอบที่บันทึกข้อมูลต้องใช้แนวทางการดูแลแตกต่างจากองค์ประกอบที่ไม่บันทึกข้อมูล
...
- **แนวคิดหลัก:** Ingress ทำหน้าที่เป็นทางเข้าหลักเช่นเดียวกับ reverse proxy ที่ศึกษาแล้ว โดยรับข้อกำหนดจาก Kubernetes
...
- **แนวคิดหลัก:** การเปลี่ยนเวอร์ชันทีละส่วนช่วยรักษาความต่อเนื่องของบริการแก่ผู้ใช้
...
- **แนวคิดหลัก:** Kubernetes ต้องทราบปริมาณทรัพยากรที่แอปพลิเคชันใช้ เพื่อเลือก node ที่เหมาะสมและป้องกันไม่ให้ workload หนึ่งใช้ทรัพยากรทั้งหมด
...
- **แนวคิดหลัก:** อาการที่ Kubernetes แสดงสามารถใช้ระบุชั้นที่เกิดปัญหาได้
...
## 021-the-big-picture
- **แนวคิดหลัก:** องค์ประกอบทั้งหมดที่ศึกษาเชื่อมโยงกันเป็นระบบเดียว
