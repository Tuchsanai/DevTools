...
import { getSystemStatus, runtimeConfig } from "./lib/runtime";
import {
  IconAlert,
  IconBoard,
  IconCheck,
...
import { SystemStatusCard } from "./ui/system-status";

export const dynamic = "force-dynamic";

export async function generateMetadata(): Promise<Metadata> {
  const { siteName } = runtimeConfig();
  return {
    title: `${siteName} · ระบบงานซ่อมและครุภัณฑ์`,
    description: `ระบบแจ้งซ่อม ยืม-คืนครุภัณฑ์ และคลังอะไหล่ของ ${siteName}`,
  };
}
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const [h, counts, status] = await Promise.all([headers(), loadCounts(), getSystemStatus()]);
  const siteName = status.site_name;
  ...
  return (
    <html lang="th" data-theme={status.theme}>
        ...
        <div className="flex min-h-screen">
              ...
              <p className="flex items-center gap-2 text-[13px] leading-[1.5] text-zinc-400">
                <span
                  className={`h-1.5 w-1.5 shrink-0 rounded-full ${
                    !status.api.configured ? "bg-zinc-500" : counts.ready ? "bg-ok" : "bg-crit"
                  }`}
                  aria-hidden="true"
                />
                {!status.api.configured
                  ? "โหมดเดี่ยว — พร้อมเชื่อมต่อ API"
                  : counts.ready
                    ? "เชื่อมต่อบริการเบื้องหลังแล้ว"
                    : "ติดต่อบริการเบื้องหลังไม่ได้"}
              </p>
            </div>
          </aside>
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            ...
            <header className="flex items-center gap-4 border-b border-rule bg-card px-6 py-2 lg:hidden">
              <span className="text-[15px] font-bold tracking-tight">{siteName}</span>
            ...
            <footer className="mx-auto w-full max-w-[92rem] px-6 pb-5 lg:px-8">
              <p className="border-t border-rule pt-3 text-[13px] text-ink-3">
                {siteName} · ทุกการกดปุ่มคือ form POST ที่ประมวลผลฝั่งเซิร์ฟเวอร์ ·
                หน้าเว็บคุยกับบริการเบื้องหลังผ่าน Service ภายใน cluster
              </p>
            </footer>
          </div>
        </div>
