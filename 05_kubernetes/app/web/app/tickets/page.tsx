...
export default async function TicketsPage({
  ...
  if (!data) return <OfflineTicketsPage tone={sp.t} message={sp.m} />;
  const [tickets, allTickets, assets, parts, dash] = data;
...
function OfflineTicketsPage({ tone, message }: { tone?: string; message?: string }) {
  return (
    <>
      <Flash tone={tone} message={message} />
      <PageHead eyebrow="กระดานงานซ่อม" title="บันทึกอาการไว้เมื่อบริการกลับมาพร้อม" />
      <Panel>
        <PanelHead
          title="แจ้งซ่อมใหม่"
          sub="ฟอร์มยังตอบสนองได้ในโหมดเดี่ยว แต่จะไม่บันทึกจนกว่า API และฐานข้อมูลพร้อม"
        />
        <form action={createTicketAction} className="grid gap-4 px-6 py-5 lg:grid-cols-12">
          <input type="hidden" name="back" value="/tickets" />
          <div className="lg:col-span-2">
            <label className={labelClass} htmlFor="asset_id">
              รหัสครุภัณฑ์
            </label>
            <input id="asset_id" name="asset_id" type="number" min={1} defaultValue={1} className={inputClass} />
          </div>
          <div className="lg:col-span-3">
            <label className={labelClass} htmlFor="title">
              หัวข้อ
          ...
          <div className="lg:col-span-4">
            ...
            <input id="detail" name="detail" className={inputClass} />
          </div>
          <div className="lg:col-span-2">
            <label className={labelClass} htmlFor="priority">
              ความเร่งด่วน
        ...
        <p className="border-t border-rule bg-wash px-6 py-3 text-[13px] text-ink-3">
          สถานะ: รอเชื่อมต่อ API · การกดส่งจะแสดงสาเหตุบนหน้าเดิมโดยไม่ทำให้เว็บล่ม
        </p>
      </Panel>
    </>
  );
