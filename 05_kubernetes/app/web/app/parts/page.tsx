...
import { UnavailablePage } from "../ui/unavailable";
...
export default async function PartsPage({
  ...
  const parts = await apiGet<Part[]>("/api/parts").catch(() => null);
  if (!parts) return <UnavailablePage title="คลังอะไหล่" />;
  ...
  if (!moveRows) return <UnavailablePage title="คลังอะไหล่" />;
  const movesByPart = new Map<number, StockMove[]>(moveRows);
