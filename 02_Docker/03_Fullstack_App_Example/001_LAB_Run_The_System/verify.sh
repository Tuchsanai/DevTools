...
# สคริปต์นี้สร้าง/ลบเฉพาะของตัวเอง : container ชื่อขึ้นต้น vops1- และ volume ชื่อ vops1-*
...
cleanup() {
  docker rm -f -v vops1-db1 vops1-db2 vops1-db3 vops1-db4 vops1-db5 vops1-nopass >/dev/null 2>&1
...
docker run -d --name vops1-nopass postgres:17-alpine >/dev/null 2>&1
sleep 3
nopass_state=$(docker inspect -f '{{.State.Status}}:{{.State.ExitCode}}' vops1-nopass 2>/dev/null)
if docker logs vops1-nopass 2>&1 | grep -q 'You must specify POSTGRES_PASSWORD'; then
...
docker rm -f -v vops1-nopass >/dev/null 2>&1

# ---------- 3) initdb สร้างตารางครบ 5 ตาราง ----------
run_db vops1-db1
if wait_pg vops1-db1; then
  pass "กล่อง vops1-db1 ขึ้นและรับ connection ได้"
else
  fail "กล่อง vops1-db1 ไม่พร้อมรับ connection ภายใน 40 วินาที"
  echo "$failures CHECK(S) FAILED"
  exit 1
fi

tables=$(docker exec vops1-db1 psql -U opsuser -d campusops -tAc \
...
if docker logs vops1-db1 2>&1 | grep -q 'running /docker-entrypoint-initdb.d/01-schema.sql'; then
  pass "log บอกว่ารันไฟล์ /docker-entrypoint-initdb.d/01-schema.sql จริง"
else
  fail "ไม่พบบรรทัด 'running /docker-entrypoint-initdb.d/01-schema.sql' ใน log ของ vops1-db1"
fi
...
low=$(docker exec vops1-db1 psql -U opsuser -d campusops -tAc \
...
docker rm -f -v vops1-db1 >/dev/null 2>&1
run_db vops1-db2
if wait_pg vops1-db2; then
  lost=$(count_of vops1-db2 tickets)
  if [ "$lost" = "8" ]; then
  ...
  fail "กล่อง vops1-db2 ไม่พร้อมรับ connection"
fi
docker rm -f -v vops1-db2 >/dev/null 2>&1

# ---------- 6) มี named volume → ข้อมูลอยู่ ----------
docker volume rm vops1-pgdata >/dev/null 2>&1
run_db vops1-db3 -v vops1-pgdata:/var/lib/postgresql/data
if wait_pg vops1-db3; then
  docker exec vops1-db3 psql -U opsuser -d campusops -c \
    "INSERT INTO tickets (asset_id, title, detail, priority) VALUES (4, 'ตรวจงานอัตโนมัติ', 'แถวที่ verify.sh เพิ่มเอง', 'HIGH');" >/dev/null 2>&1
  kept_before=$(count_of vops1-db3 tickets)
  ...
  fail "กล่อง vops1-db3 ไม่พร้อมรับ connection"
fi
...
run_db vops1-db4 -v vops1-pgdata:/var/lib/postgresql/data
if wait_pg vops1-db4; then
  kept_after=$(count_of vops1-db4 tickets)
  ...
  fail "กล่อง vops1-db4 ไม่พร้อมรับ connection"
fi

# ---------- 7) init script ต้องไม่รันซ้ำเมื่อ volume ไม่ว่าง ----------
if docker logs vops1-db4 2>&1 | grep -q 'Skipping initialization'; then
  pass "volume ไม่ว่าง : log ขึ้น 'Skipping initialization' — init script ถูกข้าม"
else
  fail "คาดว่า log ของ vops1-db4 ต้องมี 'Skipping initialization' แต่ไม่พบ"
fi

if docker logs vops1-db4 2>&1 | grep -q 'running /docker-entrypoint-initdb.d/02-seed.sql'; then
  fail "vops1-db4 รัน 02-seed.sql ซ้ำ ซึ่งไม่ควรเกิดเมื่อ volume ไม่ว่าง"
else
  pass "vops1-db4 ไม่ได้รัน 02-seed.sql ซ้ำ ข้อมูลจึงไม่ถูกเติมซ้ำซ้อน"
fi
docker rm -f -v vops1-db4 >/dev/null 2>&1

# ---------- 8) --env-file ให้ผลเท่ากับ -e หลายตัว ----------
docker run -d --name vops1-db5 \
  --env-file .env.db \
  -v vops1-pgdata:/var/lib/postgresql/data \
  -v "$PWD/db/initdb:/docker-entrypoint-initdb.d:ro" \
  postgres:17-alpine >/dev/null 2>&1
if wait_pg vops1-db5; then
  envs=$(docker exec vops1-db5 env 2>/dev/null | grep -c -E '^POSTGRES_(DB|USER|PASSWORD)=')
  envfile_count=$(count_of vops1-db5 tickets)
  ...
  fail "กล่อง vops1-db5 ที่ใช้ --env-file ไม่พร้อมรับ connection"
fi
docker rm -f -v vops1-db5 >/dev/null 2>&1
