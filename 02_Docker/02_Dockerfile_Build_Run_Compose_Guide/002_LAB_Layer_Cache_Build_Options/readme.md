...
> ไฟล์ในโฟลเดอร์นี้ : `Dockerfile.good` (ลำดับที่ดี) · `Dockerfile.bad` (ลำดับที่ควรเลี่ยง) · `app.py` (Flask หน้าเดียว) · `requirements.txt` · `.dockerignore` · `verify.sh` (ตัวตรวจอัตโนมัติ) · `test_logs/` (log ดิบของการรันจริงที่ทุก Expected output ในเอกสารนี้อ้างอิง) · `images/`
...
> ⚠️ **จุดที่เปลี่ยนไปใน Docker 29 :** เอกสารเก่า ๆ บอกว่า "build ทับ tag เดิมแล้วจะเหลือ image `<none>` ค้างเต็มเครื่อง" — **บน Docker 29 ที่ใช้ containerd image store ไม่เป็นแบบนั้นแล้ว** ลอง build ทับ `-t cachelab-good:1.0` ซ้ำกี่รอบก็ไม่มี `<untagged>` งอกออกมา (ทดสอบจริงแล้วใน `test_logs/19_dangling_prune.log`) · วิธีทำให้เกิด dangling image บนเวอร์ชันนี้คือ **build โดยไม่ใส่ `-t`** อย่างที่เราเพิ่งทำ · ถ้าเครื่องของคุณ Docker เก่ากว่านี้จะเห็น `<none>` งอกจากการ build ทับ tag เดิมตามคู่มือเก่า — ทั้งสองแบบใช้ `docker image prune` เก็บกวาดเหมือนกัน
...
```bash
docker build -t myapp:1.0                 # ลืม build context (จุดท้ายคำสั่ง)
docker build . -t myapp:1.0 extra         # มี argument เกินมา 2 ตัว
docker build -f Dockerfile.prod -t myapp:1.0 .   # -f ชี้ไฟล์ที่ไม่มีอยู่
```

> 📝 **คำอธิบาย:** สองบรรทัดแรกคือความผิดพลาดอันดับหนึ่งของคนเริ่มใช้ Docker — Docker ไม่เดาให้ว่าเราหมายถึงโฟลเดอร์ปัจจุบัน และ **context มีได้ตัวเดียวและต้องอยู่ท้ายสุด** · บรรทัดที่สามคือเคสที่เจอบ่อยพอกัน คือพิมพ์ชื่อไฟล์ผิดตัวอักษรเดียว หรือสั่ง build จากคนละโฟลเดอร์กับที่คิด (ในโฟลเดอร์นี้มีแต่ `Dockerfile.good` กับ `Dockerfile.bad`)

✅ **Expected output** — สองบรรทัดแรกให้ error เดียวกันและยังไม่ทันเริ่ม build เลย ส่วนบรรทัดที่สาม **เริ่ม build ไปแล้ว** (มีขั้น `#1`) ก่อนจะพังตอนอ่านไฟล์ · ทั้งสามคืน exit code 1:
...
> 📝 **แปล error ให้เป็น:** `requires 1 argument` = "ต้องมี argument 1 ตัว" ซึ่งก็คือ build context ที่เราลืมไป และบรรทัด `Usage:` ที่ตามมาคือคำใบ้ว่า `PATH | URL | -` ต้องอยู่**ท้ายสุด** → **แก้โดยเติม `.` ต่อท้าย** · ส่วน `transferring dockerfile: 2B` แปลว่า "ไฟล์เปล่า" เป็นสัญญาณแรกว่าผิดปกติ แล้ว `open Dockerfile.prod: no such file or directory` ก็บอกชัดว่าหาไฟล์ไม่เจอ → **แก้โดย `ls Dockerfile*` แล้วสะกดให้ตรง** · ระวังว่า path ของ `-f` อ้างอิงจาก**โฟลเดอร์ที่เรายืนอยู่** ไม่ใช่จาก build context
...
- [ ] ทำให้พังครบ 2 แบบ (ลืม build context · `-f` ชี้ไฟล์ที่ไม่มี) แล้วอ่าน error ออกว่าต้องแก้ตรงไหน
