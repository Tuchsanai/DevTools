#!/usr/bin/env python3
"""ใส่ marker (กรอบแดง + เลขลำดับ + ป้ายไทย) ลงภาพหน้าจอ UI ตามสเปกใน tools/ui/annotations/labN.json"""
...
# ค่าฐานปรับไว้สำหรับภาพกว้าง 1440 px — ภาพที่ใหญ่กว่านั้นถูกขยาย marker ตามสัดส่วน
BASE_WIDTH = 1440
LINE_WIDTH = 5
FONT_SIZE = 28
MASK_PADDING = 4
...
def scale_for(width: int) -> float:
    """marker บนภาพความละเอียดสูงต้องใหญ่ตามภาพ ไม่งั้นมองแทบไม่เห็น"""
    return max(1.0, width / BASE_WIDTH)


def apply_scale(factor: float) -> None:
    global LINE_WIDTH, MASK_PADDING, LABEL_PADDING_X, LABEL_PADDING_Y, LABEL_RADIUS
    global ARROW_WIDTH, ARROW_LENGTH
    LINE_WIDTH = max(5, round(5 * factor))
    MASK_PADDING = max(4, round(4 * factor))
    LABEL_PADDING_X = max(14, round(14 * factor))
    LABEL_PADDING_Y = max(8, round(8 * factor))
    LABEL_RADIUS = max(10, round(10 * factor))
    ARROW_WIDTH = max(4, round(4 * factor))
    ARROW_LENGTH = max(15, round(15 * factor))
...
def load_fonts(repo: Path, size: int) -> tuple[ImageFont.FreeTypeFont, ImageFont.FreeTypeFont]:
    ...
    symbol_path = Path("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf")
    if not symbol_path.is_file():
        raise FileNotFoundError(f"circled-number fallback font not found: {symbol_path}")
    return (
...
def arrow(draw: ImageDraw.ImageDraw, start: tuple[float, float], end: tuple[float, float]) -> None:
    draw.line((start, end), fill=ROSE, width=ARROW_WIDTH)
...
def annotate_image(
    ...
    factor = scale_for(width)
    apply_scale(factor)
    fonts = load_fonts(repo, round(FONT_SIZE * factor))
    ...
    for index, shape in enumerate(shapes, start=1):
        ...
        text = shape["label"]
        if len(text) > 30:
            raise ValueError(f"label exceeds 30 characters: {text!r}")
...
def spec_paths(repo: Path, selection: str) -> list[Path]:
