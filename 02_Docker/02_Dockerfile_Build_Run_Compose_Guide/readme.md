...
- **[`dockerfile_teaching_Slides.html`](dockerfile_teaching_Slides.html)** — สไลด์ 16:9 **13 หน้า หน้าละ 1 ตอน** ย่อจาก [`dockerfile_build_run_compose_guide.html`](dockerfile_build_run_compose_guide.html) (เปิดในเบราว์เซอร์ได้เลย ไฟล์เดียว 0.47 MB ไม่มี CDN)
  - `←`/`→`/`↑`/`↓`/`PageUp`/`PageDown`/`Space` เปลี่ยนหน้า · `Home`/`End` ไปหน้าแรก/สุดท้าย · `1`–`9` ไปหน้านั้น · `O` หรือ `G` ดูภาพรวม 13 ตอน · `Esc` ปิด · ปัดซ้าย/ขวาบนจอสัมผัสได้ · `Ctrl+P` พิมพ์ได้หน้าละแผ่น
  - เลขหน้ามุมล่างขวาทุกหน้า · ปุ่มควบคุมซ่อนตัวเมื่อไม่ขยับเมาส์เพื่อไม่ทับเนื้อหา
  - ภาพประกอบ Excalidraw 8 รูป + ภาพผลการทดลองจริง 2 รูป ฝังเป็น base64 ในไฟล์
  - แก้เนื้อหาที่ [`slides_build/`](slides_build/README.md) แล้ว `python3 build_slides.py` — **อย่าแก้ไฟล์สไลด์ตรง ๆ**
